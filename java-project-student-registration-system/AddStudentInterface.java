...
/**
 * AddStudentInterface class instantiates and manages a set of graphical user
...
public class AddStudentInterface extends JFrame
{
	...
	public AddStudentInterface(EventListener listener)
	{
		...
		String[] courseLabelArray =
				{"Course Credit Hours", "Course Name", "course Id",
						"Instructor Name", "Scheduled Days", "Scheduled Room", "Scheduled Time"};
	...
	public boolean inputIsValid()
	{
		...
		try {
			...
			infoIsValid = false;
			errorMessage = errorMessage.concat
				("Registration date must be in the format DD/MM/YYYY\n");
		} // end try-catch block
		
		// Check that Phone Number is in the format ###-###-####.
		if (!studentFields[3].getText().matches("[0-9]{10}"))
		{
			infoIsValid = false;
			errorMessage = errorMessage.concat
		...
		if (!(studentFields[6].getText().matches("[0-9]{5}")))
		{
			infoIsValid = false;
			errorMessage = errorMessage.concat
				("Zip Code must be in one of these formats: 12345 \n");
		} // end if
	...
	{
		this.course = c;
		courseFields[0].setText("" + c.getCreditHours());
		courseFields[1].setText(c.getCourseName());
		courseFields[2].setText(c.getcourseId());
		courseFields[3].setText(c.getInstructorName());
		courseFields[4].setText(c.getScheduledDays());
		courseFields[5].setText(c.getScheduledRoom());
		courseFields[6].setText(c.getScheduledTime());
