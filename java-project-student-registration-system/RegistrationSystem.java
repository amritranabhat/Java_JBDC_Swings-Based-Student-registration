...
public class RegistrationSystem implements ActionListener, WindowListener
{
	...
	{
		String[][] textDump = new String[getNumberOfStudents()][8];
		for (int i = 0; i < getNumberOfStudents(); i++)
		{
			...
			textDump[i][3] = students[i].getPhoneNumber();
			textDump[i][4] = students[i].getCurrentCity();
			textDump[i][5] = students[i].getCurrentState();
			textDump[i][6] = students[i].getCurrentZipCode();
			textDump[i][7] = String.valueOf(students[i].getRegistrationDate());
		} // end for
